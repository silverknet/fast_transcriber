...
- 🔧 Step 4 (in progress) — Migrate `WaveformPlayer.svelte` to the controller; delete the local click loop / `audioTransport` import / click toolbar UI. Recommended phased approach is in `HANDOFF_FOR_CODEX.md`.
  ...
  - ⏳ **Phase D** — `transport.X(tbind())` still drives currentTime mirror + range-end auto-stop. Migrate to controller's `currentTime` / `isPlaying` + `#tickTransport`. Risk: WaveformPlayer's `isPlaying` / `currentTime` are written from a dozen places via `tbind`; flipping to `$derived` requires deleting every writer at once (the previous attempt got tangled there). Take this as its own focused diff. The handoff doc has notes on the right shape.
  ...
  - **Behavioral note** — in this state the controller's click loop fires for grid-mode play, AND the audioTransport rAF runs in parallel mirroring `el.currentTime` into WaveformPlayer's local `currentTime` state. Slight overhead (two rAFs) but correct. The controller's `play()` mid-song-guard (`audio.currentTime <= songStart + 0.05`) replaces the equivalent check that used to live in WaveformPlayer's `togglePlay`.
