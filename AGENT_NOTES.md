...
- 🔧 Step 4 (in progress) — Migrate `WaveformPlayer.svelte` to the controller; delete the local click loop / `audioTransport` import / click toolbar UI. Recommended phased approach is in `HANDOFF_FOR_CODEX.md`.
  ...
  - ✅ **Phase A** — controller instantiated in parent; songMap / rangeStart / rangeEnd / volumes fed via `$effect`s; WaveformPlayer hands it the `<audio>` element. `controller.playWithClick` deliberately NOT synced (dormant) so its click loop doesn't fight WaveformPlayer's working loop. Behavior unchanged; sets up phases B-E + Step 8.
  - ⏳ **Phase B-E** — route play through controller, delete local click loop, delete audioTransport, move toolbar.
