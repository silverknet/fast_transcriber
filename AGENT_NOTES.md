...
- ⏳ Step 6a — Count-in ghost ticks + per-bar "Set as start" in `TimelineBeatGrid.svelte`.
- ⏳ Step 6b — Reset-grid affordance (NEW, user-requested 2026-06-06). Schema delta: `timeline.original?: { bars: Bar[]; beats: Beat[] }`, captured at analysis time. Toolbar button restores the original; disabled when no snapshot or when current matches snapshot. Survives reload (lives in `.smap`). Bridges the gap until full undo/redo lands later.
- ⏳ Step 7 — `spokenIntroText` input in the Cue tab.
