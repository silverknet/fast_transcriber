...
export type BassNoteRaw = { timeSec: number; durationSec: number; midi: number; velocity: number }

export type AnalyzeBassResult =
  | {
      ok: true
      notes: BassNoteRaw[]
      analyzerVersion: number
...
/**
 * Transcribe bass notes from an on-disk bass stem (the sidecar reads the file
 * directly — no audio over HTTP). Times are ORIGINAL audio time, same base
 * as `Beat.timeSec`.
 */
export async function analyzeBassViaDesktop(
  stemAbsPath: string,
  signal?: AbortSignal,
): Promise<AnalyzeBassResult> {
  try {
    const res = await fetch(`http://127.0.0.1:${BARBRO_DESKTOP_BEACON_PORT}/native/analyze-bass`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ stemAbsPath }),
      signal,
    ...
    const d = payload.data
    const notes: BassNoteRaw[] = []
    if (Array.isArray(d.notes)) {
      for (const ev of d.notes) {
        if (!ev || typeof ev !== 'object') continue
        const e = ev as Record<string, unknown>
        const t = typeof e.timeSec === 'number' && Number.isFinite(e.timeSec) ? e.timeSec : null
        const dur =
          typeof e.durationSec === 'number' && Number.isFinite(e.durationSec) ? e.durationSec : null
        const midi = typeof e.midi === 'number' && Number.isInteger(e.midi) ? e.midi : null
        const v = typeof e.velocity === 'number' && Number.isFinite(e.velocity) ? e.velocity : 1
        if (t === null || t < 0 || dur === null || !(dur > 0)) continue
        if (midi === null || midi < 0 || midi > 127) continue
        notes.push({ timeSec: t, durationSec: dur, midi, velocity: Math.max(0, Math.min(1, v)) })
      }
    }
    return {
      ok: true,
