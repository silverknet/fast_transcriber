...
| SE | Generated drum track (BarBro Drums) | M | Drum stem → band-flux hit detection (`sections/transcribe_drums.py`, tuned on 3 real songs: min-tail cymbal discrimination, ghost-kick suppression) → `drumMidi` events (synced, LWW) → mixer lane "BarBro Drums" rendered with a picker of two kits (deterministic synth + acoustic drop-in/CC0 with synth fallback) at fixed −16 dB RMS. Quantize off/8ths/16ths/triplets at render time. Bass-ready schema. Gaps: no bundled acoustic samples yet, toms weak, Ableton MIDI export deferred. |
