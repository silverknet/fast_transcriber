...
| SE | Generated drum track (BarBro Drums) | M | Drum stem → band-flux hit detection (`sections/transcribe_drums.py`, tuned on 3 real songs: min-tail cymbal discrimination, ghost-kick suppression) → `drumMidi` events (synced, LWW) → mixer lane "BarBro Drums" rendered with a picker of two kits (deterministic synth + acoustic drop-in/CC0 with synth fallback) at fixed −16 dB RMS. Quantize off/8ths/16ths/triplets at render time. Bass-ready schema. Gaps: no bundled acoustic samples yet, toms weak, groove engine loses fills/transitions (steady patterns only), Ableton MIDI export deferred. |
| SE | Generated bass track (BarBro Bass) | M | Bass stem → YIN monophonic note tracking (`sections/transcribe_bass.py`: RMS gate, attack-flake cleanup; validated on LNFSG vs hand-placed chords — 41.6% of sounding time on the exact bass note once the sheet's +2 key offset is accounted for, rest on chord tones) → `bassMidi` notes (synced, LWW like drumMidi) → "BarBro Bass" mixer lane, plucked-string synth (harmonic stack, no reverb, center pan) at fixed −18 dB RMS. Onset quantize reuses the drum grid options; transposition shifts NOTES and re-synthesizes (no stretch artifacts). Panel row lives in the BarBro Band panel. Gaps: single voice, no groove/simplify mode, Ableton MIDI export deferred. |
