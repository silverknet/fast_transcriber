...
| SE | Generated drum track (BarBro Drums) | M | Drum stem → band-flux hit detection (`sections/transcribe_drums.py`, tuned on 3 real songs: min-tail cymbal discrimination, ghost-kick suppression) → `drumMidi` events (synced, LWW) → mixer lane "BarBro Drums" rendered with a picker of three kits (deterministic synth; acoustic drop-in/CC0 with synth fallback; "Your kit" — the user's own one-shots from `<project>/kits/drums/*.wav` read via the sidecar, never bundled, partial kits filled by fallbacks, mixer always re-synthesizes since the fingerprint can't see sample files) at fixed −16 dB RMS. Quantize off/8ths/16ths/triplets at render time. Bass-ready schema. Gaps: no bundled acoustic samples yet, toms weak, groove engine loses fills/transitions (steady patterns only), Ableton MIDI export deferred. |
