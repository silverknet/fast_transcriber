...
| SE | Chord-sheet import (UG paste) | M | Paste a chords-over-lyrics sheet in the Lyrics tab → strict token parser (`chords/sheet/chordToken.ts`, lenient-parser trap locked by tests), column→word anchoring (`parseChordSheet.ts`), placement via timed lyric words + beat snap (`placeChords.ts` + `songmap/beatAtTime.ts`), instrumental spreads, sections derived when song has none (`deriveSections.ts`). One undo step; import stashes existing chords as a parallel track (v5 `chordLayers`, switcher in the Chords tab). URL fetch + chroma advisory deferred. |
