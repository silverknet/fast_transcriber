...
| SE | Chord-sheet import (UG paste) | M | Paste a chords-over-lyrics sheet in the Lyrics tab → strict token parser (`chords/sheet/chordToken.ts`, lenient-parser trap locked by tests), column→word anchoring (`parseChordSheet.ts`), placement quantizes anchors to bar downbeats with a capped per-bar chroma vote + shared-bar-penalty DP (`placeChords.ts`; 12/14 exact-beat vs a hand-placed LNFSG reference, was 2/14), instrumental spreads, sections derived from markers (`deriveSections.ts`). One undo step; an import lands as a new **song draft** (v6 `drafts[]` — sections + chords + lyrics as one unit), preserving the previous draft. Switcher sits next to the song title. URL fetch deferred. |
...
| DT | Browser-only consumer/collab mode | S | Two-mode split ([`desktop-vs-browser.md`](domains/desktop-vs-browser.md)) — **reverses** the old "desktop-required" stance. Creators need the sidecar (analyze/stems/transcribe/TTS/YouTube/local folder + local **HD** audio); consumers/collaborators run **browser-only on compressed cloud audio** (AAC/m4a ~128k, mix+stems). **Landed:** audio-fidelity failsafe [`resolveAudioSource.ts`](../src/lib/audio/resolveAudioSource.ts) (local HD always wins, cloud unreachable on desktop; unit-tested), [`appMode`](../src/lib/stores/appMode.ts), navbar mode badge in [`AppMenuBar`](../src/lib/components/AppMenuBar.svelte); browser-mode **live collaboration on `.smap`** — sends AND now RECEIVES chord/lyric/section/draft edits live ([`browserCloudPull.ts`](../src/lib/client/browserCloudPull.ts): in-memory pull worker reached via a dispatch branch in `pullCloudChanges`, undo-safe [`patchSongMapRemote`](../src/lib/stores/songMap.ts), a collaborator's new song appears live), plus a symmetric dangerous-conflict dialog on the PULL path (fixes a silent-clobber on desktop too); merge primitives and the `.smap` root-of-truth make the sidecar irrelevant to text/chord sync. **Pending:** Supabase Storage bucket + RLS + `cloud_songs` audio cols, sidecar AAC transcode + upload, `cloudAudio.ts` (download/IndexedDB cache), `CloudProjectStore` (no disk folder), browser join, sidecar-action gating. |
...
**Epic rollup:** **M** — `.smap` + parsing strongest (**R**); cloud / sync now **M** (auth + schema + sync engine + audio reconcile + hydration packs + status pill + conflict modal all shipped; merge + cloud-roundtrip now covered by [`draftsSharing.test.ts`](../src/lib/songmap/draftsSharing.test.ts), including two-device migration determinism and cross-draft conflict resolution. Remaining gap to **R**: no realtime pull while `/edit` is open — a pull writes disk but not the open editor, so live co-editing of the same song is not yet safe).
...
| PF | Cloud / sync (auth + project collab) | M | Supabase auth ([`hooks.server.ts`](../src/hooks.server.ts), Google OAuth + magic link, admin access panel now merges Auth-only users missing `access_grants` while excluding env-admins) ✓; cloud schema ([`004_drop_legacy`](../db/migrations/004_drop_legacy.sql)–[`010_cloud_rpcs`](../db/migrations/010_cloud_rpcs.sql)) ✓; sync engine + `/api/cloud/projects/[id]/(songs\|members\|manifest)` + [`cloudSync.ts`](../src/lib/client/cloudSync.ts) ✓; **Phase 5+6 audio reconcile** ([`audioReconcile.ts`](../src/lib/project/audioReconcile.ts) + sidecar `/native/project/song/audio/scan` + Locate / Import-pack / Ignore in [`RelinkAudioBanner.svelte`](../src/lib/components/RelinkAudioBanner.svelte)) ✓; **hydration packs** ([`AppMenuBar`](../src/lib/components/AppMenuBar.svelte) → File → Export/Import; `/native/project/hydration/{export,import}`) ✓; **Phase 7 status pill** ([`CloudSyncPill.svelte`](../src/lib/components/CloudSyncPill.svelte) — Synced / N pending / Offline; manual retry; auto-flush on `online` event) ✓; **Phase 8 conflict modal** ([`collabMerge.ts`](../src/lib/songmap/collabMerge.ts) field classification + [`ConflictResolutionDialog.svelte`](../src/lib/components/ConflictResolutionDialog.svelte) with per-row Keep mine / Take theirs and a "Take theirs (all)" shortcut) ✓; **draft-aware merging** — root `sections`/`harmony`/`lyrics` are the active draft's content, so per-id merging applies only when both sides are on the SAME draft; divergent drafts resolve wholesale with the losing draft preserved ([`draftsSharing.test.ts`](../src/lib/songmap/draftsSharing.test.ts)) ✓; **recording identity** ([`audioFingerprint.ts`](../src/lib/audio/audioFingerprint.ts)) — a loudness-envelope fingerprint that tells "same master, different encoding" (fine) from "different cut" (breaks the grid), surfaced by [`RecordingMismatchBanner.svelte`](../src/lib/components/RecordingMismatchBanner.svelte) ✓. **realtime actually works now** — `cloud_songs`/`cloud_projects`/`cloud_project_members` were never in the `supabase_realtime` publication, so auto-pull had never fired and sync was push-only; fixed by [`015_realtime_publication.sql`](../db/migrations/015_realtime_publication.sql) and guarded by `npm run check:realtime` ✓; auto-pull moved out of `/project` into app-wide [`cloudAutoPull.ts`](../src/lib/client/cloudAutoPull.ts) with remote changes landing in memory via [`songSession.ts`](../src/lib/project/songSession.ts) ✓. Remaining: `reconcileSongAudio` can't use recording identity until the sidecar computes it; LWW merge is being replaced per [`collab-sync-architecture.md`](domains/collab-sync-architecture.md) — **Phase 2 landed** as a SHADOW representation ([`ydoc.ts`](../src/lib/songmap/ydoc.ts): `songMapToYDoc` / `yDocToSongMap`, seeded under a pinned `clientID` so two devices seeding the same `.smap` converge instead of clobbering each other). Nothing in the app reads it yet; writers migrate in Phase 3. |
...
- **Cloud / sync (`M`)**  
  ...
  **The dialog is now rare.** It fired on ordinary first-open, because a local `.smap` and its cloud row can sit at different legacy `formatVersion`s (observed: `4` on disk, `2` in the cloud) and migrating both to v6 from different starting points diverges honestly. The 409 handler in [`projectAutosave.ts`](../src/lib/client/projectAutosave.ts) now settles anything with no `dangerous` conflict itself — rebasing onto the server revision, re-pushing and adopting the result — and only prompts for whole-timeline replacement, wholesale chord-track swap, a divergent active draft, an `metadata.analyzed` flip, or an audio-identity swap. Users hear about it afterwards through the existing coalescing notice ([`cloudToast.ts`](../src/lib/stores/cloudToast.ts), `reconciled` wording) instead of being blocked.

  Auto-settling uses `autoResolvedMerge`, **not** the raw cloud-wins default: cloud sync was push-only-on-open for a long time, so a stale row can be missing whole fields, and letting it win would delete work that never reached the server (a v2 row predates both `transpose` and `lyrics`). Where the cloud value is empty and the local one is not, the local value is kept — provably lossless, since the cloud had nothing to lose. Two non-empty sides, and every id-keyed item, still resolve cloud-wins.
  ...
  **Automated coverage**:
    ...
    - [`collabAutoResolve.test.ts`](../src/lib/songmap/collabAutoResolve.test.ts) — what still opens the dialog, the empty-cloud-never-wins rule, co-editing LWW left intact, determinism, and a version-skew regression built from a v2-derived and a v4-derived copy of one song.
