...
import { F as mergeAnalysisIntoSongMap, O as songMap, T as patchSongMap } from "../../../chunks/commit.js";
	...
	$$renderer.component(($$renderer) => {
		...
		async function run() {
			...
			try {
				...
				if (trimmedWav.size < 5e4) throw new Error(`The trimmed audio is only ${trimmedWav.size} bytes — the trim range may be empty (start ${trim.startSec.toFixed(1)}s → end ${trim.endSec.toFixed(1)}s). Go back, drag the trim handles to cover at least 10 seconds of audible audio, and try again.`);
