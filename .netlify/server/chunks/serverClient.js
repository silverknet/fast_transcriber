import { n as private_env, r as public_env } from "./shared-server.js";
import { createServerClient } from "@supabase/ssr";
import { createClient } from "@supabase/supabase-js";
//#region src/lib/server/supabase/serverClient.ts
/**
...
function createSupabaseServerClient(cookies) {
	...
	const anon = public_env.PUBLIC_SUPABASE_ANON_KEY;
	if (!url || !anon) throw new Error("Missing PUBLIC_SUPABASE_URL / PUBLIC_SUPABASE_ANON_KEY. Set them in .env (dev) or Netlify env vars (prod).");
	return createServerClient(url, anon, { cookies: {
		getAll: () => cookies.getAll(),
		setAll: (toSet) => {
			for (const { name, value, options } of toSet) cookies.set(name, value, {
				...options,
				path: "/"
			});
		}
...
function getSupabaseServiceClient() {
	...
	if (!url || !serviceKey) throw new Error("Missing PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY (server-only).");
	serviceClient = createClient(url, serviceKey, { auth: {
...
//#endregion
export { getSupabaseServiceClient as n, createSupabaseServerClient as t };
