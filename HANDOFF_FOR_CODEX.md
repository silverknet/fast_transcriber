...
- **Step 8** — Wire cue mix preview to a second `PlaybackController`; render `mixPreviewAudioEl` for real. **Important context:** the entire cue mix preview path is currently UNREACHABLE — `prepareMixPreview()` is defined in `src/routes/edit/+page.svelte` but **never called from the template**. Confirm with Martin whether he still wants this feature or whether the Mix tab (multi-track playback) supersedes it. If superseded, delete the dead path instead of wiring it.
